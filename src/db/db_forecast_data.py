...
def from_dict_list(data: list[dict]) -> List[ForecastData]:
    ...
    for entry in data:
        ...
        obj = ForecastData(
            ...
            night_icon=_as_plain_text(entry.get("night_icon")),
            night_text=_as_plain_text(entry.get("night_text")),
            night_precip_mm=entry.get("night_precip_mm"),
            night_temp_avg=entry.get("night_temp_avg"),
            morning_icon=_as_plain_text(entry.get("morning_icon")),
            morning_text=_as_plain_text(entry.get("morning_text")),
            morning_precip_mm=entry.get("morning_precip_mm"),
            morning_temp_avg=entry.get("morning_temp_avg"),
            afternoon_icon=_as_plain_text(entry.get("afternoon_icon")),
            afternoon_text=_as_plain_text(entry.get("afternoon_text")),
            afternoon_precip_mm=entry.get("afternoon_precip_mm"),
            afternoon_temp_avg=entry.get("afternoon_temp_avg"),
            evening_icon=_as_plain_text(entry.get("evening_icon")),
            evening_text=_as_plain_text(entry.get("evening_text")),
            evening_precip_mm=entry.get("evening_precip_mm"),
            evening_temp_avg=entry.get("evening_temp_avg"),
        )
        timestamp = _entry_updated_at(entry)
...
def get_forecast():
    ...
    with get_session() as s:
        return to_dict_list(
            s.query(ForecastData).filter(ForecastData.date >= DateType.today()).all()
        )
...
def _entry_updated_at(entry: dict) -> datetime:
