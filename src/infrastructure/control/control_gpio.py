...
class GPIOControl(Control):
    def __init__(self):
        self.control_state = {"pump": None, "valve": None, "levels": []}
    ...
    def setup(self):
        ...
        # Etat de repos force a LOW apres initialisation des sorties.
    ...
    def releaseAll(self):
        all_pins = [
            self.control_state["pump"],
            self.control_state["valve"],
        ] + self.control_state["levels"]
        for pin in all_pins:
            if pin is not None:
                try:
