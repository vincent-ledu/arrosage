...
def test_minmax_temperature_precip_cache(client):
    db_weather_data.delete_weather_data_by_date(
        date.today()
    ...
    assert (
        data1 == data2
    )  # Les données doivent être identiques si elles sont mises en cache
...
def test_minmax_temperature_precip_cache_ttl(client, monkeypatch):
    ...
    assert response2.status_code in [
        201
    ]  # Devrait être 201 car les données sont rafraîchies
    data2 = response2.get_json()
    ...
    assert _without_updated_at(data1) == _without_updated_at(
        data2
    )  # Les données météo ne changent pas
    assert data1.get("updated_at") != data2.get(
        "updated_at"
    )  # mais l'horodatage doit refléter le rafraîchissement
...
def test_watering_type(client):
    ...
    assert data in [
        ...
        "reinforced",
        "high",
    ]  # Replace with actual watering types
