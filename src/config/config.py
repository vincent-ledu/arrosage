...
def save_config(config):
    ...
    if not isinstance(config["levels"], list) or not all(
        isinstance(level, int) for level in config["levels"]
    ):
    ...
    if (
        not isinstance(config["coordinates"], dict)
        or "latitude" not in config["coordinates"]
        or "longitude" not in config["coordinates"]
    ):
        raise ValueError(
            "Coordinates must be a dictionary with 'latitude' and 'longitude' keys"
        )
    if not isinstance(
        config["coordinates"]["latitude"], (int, float)
    ) or not isinstance(config["coordinates"]["longitude"], (int, float)):
        raise ValueError("Latitude and longitude must be numbers")
    if not (-90 <= config["coordinates"]["latitude"] <= 90) or not (
        -180 <= config["coordinates"]["longitude"] <= 180
    ):
        raise ValueError(
